...
def calculateScaleAndShift(mesh, resolution):
    ...
    maxdim = max(maxs[0] - mins[0], maxs[1] - mins[1], maxs[2] - mins[2])
    unit = float(maxdim / (resolution - 1))

    bounding_box = [math.floor((maxs[0] - mins[0]) / unit) + 1, math.floor((maxs[1] - mins[1]) / unit) + 1,
                    math.floor((maxs[2] - mins[2]) / unit) + 1]

    scale = [(bounding_box[0] - 1)/(maxs[0] - mins[0]), (bounding_box[1] - 1)/(maxs[1] - mins[1]), (bounding_box[2] -1)/(maxs[2] - mins[2])]

    # for i in range(3):
    #     if maxs[i] - mins[i] == maxdim:
    #         bounding_box[i] = resolution
